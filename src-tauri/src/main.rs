...
async fn download_file(client: &reqwest::Client, token: &String) -> Result<Obj, Box<dyn std::error::Error>> {
...
async fn upload_file(client: &reqwest::Client, file_path: &String) -> Result<String, Box<dyn std::error::Error>> {
...
async fn request_api_simple(client: &reqwest::Client, api: &str, token: &String) -> Result<String, Box<dyn std::error::Error>> {
    let url = Url::parse_with_params(
...
#[tauri::command]
#[allow(dead_code)]
async fn backend_load_obj(file_path: String) -> Obj {
...
#[tauri::command]
async fn backend_register_obj(file_path: String) -> String {
...
#[tauri::command]
async fn backend_restore_preprocess(token: String) {
    let client = reqwest::Client::new();
...
#[tauri::command]
async fn backend_restore_embedding(token: String) {
    let client = reqwest::Client::new();
...
#[tauri::command]
async fn backend_restore_download(token: String) -> Obj {
    let client = reqwest::Client::new();
...
#[tauri::command]
async fn backend_restore_full(file_path: String) -> Obj {
    let client = reqwest::Client::new();
...
async fn main() {
    ...
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![
            backend_restore_full,
            backend_register_obj,
            backend_restore_preprocess,
            backend_restore_embedding,
            backend_restore_download
        ])
        .menu(
