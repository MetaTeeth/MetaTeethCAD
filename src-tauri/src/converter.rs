...
pub async fn convert_obj_to_ply(_obj: &Obj<Position, u32>) -> Ply<DefaultElement> {
    ...
    v_e.properties.add(PropertyDef::new(
        "x".to_string(),
    ...
    f_e.properties.add(PropertyDef::new(
...
pub async fn convert_stl_to_ply(_stl: &IndexedMesh) -> Ply<DefaultElement> {
    let mut _ply = Ply::<DefaultElement>::new();
    // begin header
    _ply.header.encoding = Encoding::BinaryLittleEndian;
    ...
    // add vertices
    let mut vertices = Vec::new();
    for stl_v in _stl.vertices.iter() {
        let mut v = DefaultElement::new();
        v.insert("x".to_string(), Property::Float(stl_v[0]));
        v.insert("y".to_string(), Property::Float(stl_v[1]));
        v.insert("z".to_string(), Property::Float(stl_v[2]));
        vertices.push(v);
    }

    // add faces
    let mut faces = Vec::new();
    for stl_f in _stl.faces.iter() {
        let mut f = DefaultElement::new();
        f.insert(
            "vertex_indices".to_string(),
            Property::ListUInt(vec![
                stl_f.vertices[0] as u32,
                stl_f.vertices[1] as u32,
                stl_f.vertices[2] as u32,
            ]),
        );
        faces.push(f);
    }
